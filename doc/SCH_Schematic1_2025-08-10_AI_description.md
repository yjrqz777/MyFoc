...
|---|---:|---|---|
| `VBUS` | 未在图中写明额定值；电压采样按约 52.8 V 满量程设计 | 外部直流输入经 Q2 缓启动 | 三相逆变桥、TPS5450 |
...
| `VBUS` | `R93 + R94 = 15 kΩ + 15 kΩ` | `R95 = 2 kΩ` | `D24 = SMF3.3A` | `C96 = 100 nF` | `PC5 / VBUS` |

理想分压比：

```text
K = 2k / (15k + 15k + 2k) = 1/16
VADC = VIN / 16
VIN  = VADC × 16
```

按 3.3 V ADC 满量程估算，被测电压理论满量程约 `52.8 V`。考虑电阻误差、TVS 漏电、ADC 误差和过压裕量，固件中不应把 52.8 V 当作安全工作电压上限。

低通滤波近似：

```text
Rth = (30k || 2k) ≈ 1.875kΩ
fc  ≈ 1 / (2π × 1.875k × 100nF)
    ≈ 849 Hz
```
...
- `15 kΩ`：R84、R85、R87、R88、R90、R91、R93、R94。
...
2. **VBUS 额定值未标**：分压理论满量程 52.8 V 不等于安全母线额定值；MOSFET、电容、驱动器和爬电距离必须单独核定。
...
这是一个以 STM32G474RET6 为核心、由 FD6288Q 驱动六只 CJAC13TH06 MOSFET 的三相 PMSM 控制板；母线经 TPS5450 生成 5 V、RT9013/REF3033 生成数字/模拟 3.3 V、MT3608 生成 12 V；四只 1 mΩ分流器配 TLV9064 约 30 倍增益采集 IA/IB/IC/IBUS，四路 15k+15k/2k 分压以 1:16 采集三相和 VBUS，并带 Hall、NTC、电位器、CAN、SPI、UART、LCD、按键和调试接口。
