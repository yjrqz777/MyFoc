...
4. **宏定义的注释**：宏定义如需注释，必须使用 `/* ... */` 块注释，并写在宏定义后方同一行。
5. **H 文件保护符**：头文件保护符应由文件名转换为全大写下划线形式，并在首尾添加双下划线。例如 `user_time.h` 使用 `__USER_TIME_H__`。
...
| 类型 | 命名规则 | 示例 |
|------|----------|------|
| 宏定义 | 全大写+下划线；立即数宏的值使用圆括号包裹；宏注释使用 `/* ... */` 并放在同一行尾部 | `#define FRAME_SOF (0xAAu) /* 帧头 */`, `#define FRAME_EOF (0x55u) /* 帧尾 */` |
